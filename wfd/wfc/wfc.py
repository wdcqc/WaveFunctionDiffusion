...
class WFCGenerator():
    def __init__(self, map_size, connection_probs = None, tile_probs = None,
                 use_uniform_probs = False,
                 use_border_probs = False,
                 boundary_grids = None,
                 preeliminate_border = True,
                 max_saved_states = 3):
        if tile_probs is None and len(connection_probs.shape) == 2: # map input
            connection_probs, tile_probs = self.get_probs_from_map(connection_probs)
        ...
        self.base_probs = tile_probs
        
        self.use_uniform_probs = use_uniform_probs
        self.use_border_probs = use_border_probs
    ...
    def get_probs(self, grid, eps = 1e-6):
        if self.use_uniform_probs:
            return self.uniform_probs
        y, x = grid
        if self.use_border_probs:
            probs = self.map_allowed_tiles[grid] * self.uniform_probs
            if x > 0:
                next_grid = y, x-1
                if self.is_fixed(next_grid):
                    next_grid_state = self.get_first_possible_state(next_grid)
                    probs *= self.connection_probs[next_grid_state, 2] / np.maximum(eps, self.base_probs)
            if y > 0:
                next_grid = y-1, x
                if self.is_fixed(next_grid):
                    next_grid_state = self.get_first_possible_state(next_grid)
                    probs *= self.connection_probs[next_grid_state, 3] / np.maximum(eps, self.base_probs)
            if x < self.max_x - 1:
                next_grid = y, x+1
                if self.is_fixed(next_grid):
                    next_grid_state = self.get_first_possible_state(next_grid)
                    probs *= self.connection_probs[next_grid_state, 0] / np.maximum(eps, self.base_probs)
            if y < self.max_y - 1:
                next_grid = y+1, x
                if self.is_fixed(next_grid):
                    next_grid_state = self.get_first_possible_state(next_grid)
                    probs *= self.connection_probs[next_grid_state, 1] / np.maximum(eps, self.base_probs)
            if np.sum(probs) == 0:
                probs = self.map_allowed_tiles[grid] * self.uniform_probs
        else:
            probs = self.map_allowed_tiles[grid] * self.base_probs
...
#                     self.reopen_zero_grids(mapping_region, initial_state = initial_state, area_size = 0)
            ...
            if stalled_iterations >= stall_wait:
                ...
                return False
#                 mapping_region[picked_grid][:] = False
#                 self.reopen_zero_grids(mapping_region, initial_state = initial_state, area_size = 1)
#                 stalled_iterations = 0
    ...
    def generate_by_part(self,
        ...
        if param_scheduler is None:
            ...
            # param_scheduler = lambda retries: (500 + 500 * retries, 30 + 30 * retries)
